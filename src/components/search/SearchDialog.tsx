      ...
      <Command shouldFilter={false} className="border-none bg-transparent flex-1">
        ...
        <div className="flex items-center gap-6 px-4 py-2 border-b border-white/5 bg-neutral-900/40 text-[11px] uppercase tracking-widest font-semibold z-50">
          ...
          <div className="relative">
            ...
            >
              Category: <span className="text-amber-500">{selectedCategory || 'All'}</span>
            ...
            {activeDropdown === 'category' && (
              <div className="absolute top-full left-0 mt-2 w-56 bg-neutral-950 border border-white/10 rounded-lg shadow-xl py-1 z-50 overflow-hidden">
                <button 
                  ...
                  className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-amber-500/10 text-left text-neutral-300 transition-colors"
                >
                  ALL CATEGORIES
                  {!selectedCategory && <Check size={14} className="text-amber-500" />}
                </button>
          ...
          <div className="relative border-l border-white/10 pl-6">
            <button 
              ...
              className="flex items-center gap-1.5 text-neutral-400 hover:text-white transition-colors"
            >
              Tag: <span className="text-orange-500">{selectedTag || 'Any'}</span>
            ...
            {activeDropdown === 'tag' && (
              <div className="absolute top-full left-0 mt-2 w-56 bg-neutral-950 border border-white/10 rounded-lg shadow-xl py-1 z-50 overflow-hidden max-h-60 overflow-y-auto custom-scrollbar-mini">
                <button 
                  ...
                  className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-orange-500/10 text-left text-neutral-300 transition-colors"
                >
                  ANY TAG
                  {!selectedTag && <Check size={14} className="text-orange-500" />}
                </button>
                {availableFilters.tags && Object.keys(availableFilters.tags).map(tag => (
                  <button 
                    key={tag}
                    onClick={() => { setSelectedTag(tag); setActiveDropdown(null); }}
                    className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-orange-500/10 text-left text-neutral-300 transition-colors lowercase"
                  >
                    #{tag}
                    {selectedTag === tag && <Check size={14} className="text-orange-500" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <CommandGroup>
            {results.map((r) => (
              <CommandItem
                ...
                className="mx-2 my-1 cursor-pointer p-3 rounded-lg border border-transparent hover:border-amber-500/30 hover:bg-amber-500/10 transition-all"
              >
...
export function SearchDialog() {
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-2xl bg-neutral-950 border border-white/10 p-0 shadow-[0_0_50px_-12px_rgba(245,158,11,0.15)] overflow-hidden rounded-xl">
